...
function $(prop:string){return props[$props.name+"/"+prop as keyof typeof props]}
const style = computed(()=>{
  ...
  const border = !$("border/width") && !$("border/opacity") ? undefined
    : $("border/width")+`px solid rgba(${$("border/color")},${$("border/opacity") as number/100})`
    
  
  const Shadow = (()=>{
    const name = ($props.drop_shadow?"drop_shadow":"shadow") + "/"
    const fx = !$(name+"width") && !$(name+"opacity") ? undefined:
      `0 0 ${$(name+"width")}px rgba(${$(name+"color")},${$(name+"opacity") as number/100})`
      
    if(!fx) return
    else if(background === undefined || $props.drop_shadow) return {filter:`drop-shadow(${fx})`}
  ...
  return {
    ...
    ...{
      ...
      "--border": (props['menu/dev_tools'] && border) ? $("border/width")+"px" :null,
...
.panel{
  ...
  @include style("neon"){
    font-weight: 600;
    text-shadow: 0 0 .6em rgba(var(--color),.65);
